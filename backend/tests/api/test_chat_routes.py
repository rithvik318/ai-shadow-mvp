...
def test_ask_returns_an_answer_with_sources(
    ...
    response = client.post(
        "/chat", json={"question": "How does holiday accrue?"}, headers=user_headers
    )
...
def test_sources_are_ordered_most_similar_first(
    ...
    body = client.post("/chat", json={"question": "q?"}, headers=user_headers).json()
...
def test_top_k_limits_the_sources_returned(
    ...
    body = client.post(
        "/chat", json={"question": "q?", "top_k": 2}, headers=user_headers
    ).json()
...
def test_an_empty_knowledge_base_returns_200_with_no_sources(
    client: TestClient, user_headers: dict[str, str], embed_query_as, fake_llm
) -> None:
    ...
    response = client.post("/chat", json={"question": "q?"}, headers=user_headers)
...
def test_a_blank_question_is_rejected_with_422(
    client: TestClient, user_headers: dict[str, str], question: str
) -> None:
    assert (
        client.post(
            "/chat", json={"question": question}, headers=user_headers
        ).status_code
        == 422
    )


def test_a_missing_question_is_rejected_with_422(
    client: TestClient, user_headers: dict[str, str]
) -> None:
    assert client.post("/chat", json={}, headers=user_headers).status_code == 422


@pytest.mark.parametrize("top_k", [0, -1, 51])
def test_an_out_of_range_top_k_is_rejected_with_422(
    client: TestClient, user_headers: dict[str, str], top_k: int
) -> None:
    response = client.post(
        "/chat", json={"question": "q?", "top_k": top_k}, headers=user_headers
    )
...
def test_top_k_may_be_omitted(
    ...
    assert (
        client.post("/chat", json={"question": "q?"}, headers=user_headers).status_code
        == 200
    )
...
def test_the_endpoint_is_stateless(
    ...
    client.post("/chat", json={"question": "first question"}, headers=user_headers)
    client.post("/chat", json={"question": "second question"}, headers=user_headers)
